...
class MultiscaleDynamicsModelMK2(DynamicsModel, FuzzyDeviceMixin):
    ...
    def forward(self,
        ...
        for i_chunk in range(a_binned.shape[1]):
            ...
            # TODO: add the .detach() statement back here once test is done!
            prim_s = mem['prim_s'][-1].detach()
            mem['abstr_o_target'].append(prim_s)
    ...
    def eval_step(self,
        ...
        #abstr_o_target = bin_every_k_steps(pred['prim_s'], self.abstract_step_size, padding_val=0).detach()
        #abstr_o_target = abstr_o_target[:, :, -1]
        #diff = torch.sum(abstr_o_target - pred['abstr_o_target'], dim=(1, 2))
    ...
    def rollout_abstract(self,
        ...
        for t in range(n_steps):
            if t < n_groundtruth_available:
                x_posterior = torch.concat([prim_s[:, t], r[:, t], term[:, t]], dim=-1)
            else:
