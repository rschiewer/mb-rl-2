...
"""
def collect_data_torch(env: CacheLastStepEnv, n_steps: int, policy: callable = None, device: torch.device = 'cpu'):
    if policy is None:
    ...
    mem = [{'o': np.array(traj_o), 'a': np.array(traj_a), 'r': np.array(traj_r), 'terminal': np.array(traj_term),
            'truncated': np.array(traj_trunc)}]
    o, a, r, term, trunc, mask = to_tensors(mem, device)
    data = prepare_data(o, a, r, term, trunc, mask)
    return data
"""
